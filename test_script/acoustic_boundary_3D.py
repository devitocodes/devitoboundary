...
from devito import Grid, TimeFunction, Eq, solve, Operator, Substitutions, Coefficient, ConditionalDimension
...
#plt.imshow(np.swapaxes(u.data[0,:,int(model.grid.shape[1]/2),:], 0, 1),
#           origin="upper", extent=[0, model.grid.extent[0], model.grid.extent[1], 0])#,
...
#plt.imshow(np.swapaxes(u.data[1,:,int(model.grid.shape[1]/2),:], 0, 1),
#           origin="upper", extent=[0, model.grid.extent[0], model.grid.extent[1], 0])#,
           #vmin=-0.0025, vmax=0.0025)
#plt.colorbar()
#plt.xlabel("x (m)")
...
for i in range(nsnaps):
    ...
    plt.imshow(np.swapaxes(usave.data[i, PMLS:-PMLS, int(model.grid.shape[1]/2), PMLS:-PMLS], 0, 1),
               ...
               vmin=-0.005, vmax=0.005) # Just generally unfuck this
    plt.colorbar()
    plt.xlabel("x (m)")
    plt.ylabel("z (m)")
